...
def preprocess_schema(schema, imported_schemas, elements, xsd_uri, dialect, http, cache, force_download, wsdl_basedir, global_namespaces=None, qualified=False):
    ...
    for ns in list(local_namespaces.values()):
...
class OrderedDict(dict):
    ...
    def insert(self, key, value, index=0):
        if key not in self.keys2:
            self.keys2.insert(index, key)
        dict.__setitem__(self, key, value)

    def __delitem__(self, key):
        if key in self.keys2:
            self.keys2.remove(key)
        dict.__delitem__(self, key)

    def __iter__(self):
        return iter(self.keys2)

    def keys(self):
        return self.keys2

    def items(self):
        return [(key, self[key]) for key in self.keys2]

    def update(self, other):
        if isinstance(other, OrderedDict) and other.key:
            self.key = other.key
    ...
    def copy(self):
        "Make a duplicate"
        new = OrderedDict(self.key)
        new.update(self)
        return new

    def __eq__(self, other):
        return isinstance(other, OrderedDict) and self.key == other.key and self.key != None

    def __ne__(self, other):
        return not self.__eq__(other)

    def __gt__(self, other):
        if isinstance(other, OrderedDict): return (self.key[2], self.key[0], self.key[1]) > (other.key[2], other.key[0], other.key[1])
        return True
