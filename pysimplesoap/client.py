    ...
    def timeit(method):
       ...
       def timed(*args, **kw):
           ...
           with open("timings.csv", "a+") as f:
               f.write('%s %r, %2.2f \n' % \
                 (datetime.datetime.now(), args[1], te-ts))
           return result
...
<%(soap_ns)s:Body>
    ...
    def call(self, method, *args, **kwargs):
        ...
        if parameters and isinstance(parameters[0], SimpleXMLElement):
            ...
            if parameters[0].children() is not None:
                ...
                for k,v in list(parameters[0].attributes().items()):
        ...
        with open("debug/"+method + "_req.xml"+now, 'w+') as f:
            import xml.dom.minidom
            xml_req = xml.dom.minidom.parseString(self.xml_request) 
            print(xml_req.toprettyxml())
            f.write(xml_req.toprettyxml())
        with open("debug/"+method + "_res.xml"+now, 'w+') as f:
            import xml.dom.minidom
            xml_res = xml.dom.minidom.parseString(self.xml_response) 
            print(xml_res.toprettyxml()) 
            f.write(xml_res.toprettyxml()) 
    ...
    def wsdl_validate_params(self, struct, value):
        ...
        if struct == str:
            struct = str        # fix for py2 vs py3 string handling
        
        if not isinstance(struct, (list, dict, tuple)) and struct in list(TYPE_MAP.keys()):
            ...
            if struct and value:
                for key in value:
                    if key not in struct:
                        ...
                        whitelist = ['LoadAgency','GetDiamTokenForUsernamePassword','QueryForPolicyNumber','CreateNote','GetAccountBalance','GetPolicyCreditCardInfo',
                                     'GetBillingSummary','SavePolicyEftInfo']
                        if struct[key] in whitelist:
...
#WHITE LIST BRIAN
            ...
            for item in value:
                whitelist = ['LoadAgency','GetDiamTokenForUsernamePassword','QueryForPolicyNumber','CreateNote','GetAccountBalance','GetPolicyCreditCardInfo',
                             'GetBillingSummary','SavePolicyEftInfo','LoadPrintHistory','SaveUser','LoadUserLinks','GetSignedOnUser']
                if struct[key] in whitelist:
    ...
    def wsdl_parse(self, url, cache=False):
        ...
        if "http://xml.apache.org/xml-soap" in list(dict(wsdl[:]).values()): 
